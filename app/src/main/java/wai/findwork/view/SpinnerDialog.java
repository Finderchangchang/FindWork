...
public class SpinnerDialog extends Dialog {
    ...
    public void setListView(final List<CodeModel> list) {
        CommonAdapter<CodeModel>modelCommonAdapter=new CommonAdapter<CodeModel>(mContext,list,R.layout.spinner_dialog_item) {
            @Override
            public void convert(CommonViewHolder holder, CodeModel codeModel, int position) {
                holder.setText(R.id.dialog_val_tv, codeModel.getName());
            }
        };

        listView.setAdapter(modelCommonAdapter);
